...
class DANLayer(nn.Module):
    ...
    def __init__(self, inputc,hiddenc = 128):
        ...
        self.outfag = True
        sequence = []
        sequence += [GRL(),Conv(inputc,hiddenc,3,1), Conv(hiddenc,1,3,1),_ImageDA(1)]
        self.model = nn.Sequential(*sequence)
...
class DANLayer_Clip(nn.Module):
    def __init__(self, inputc,hiddenc = 128):
        super(DANLayer_Clip, self).__init__()
        self.outfag = True
        sequence = []
        sequence += [GRL(),Conv(inputc,hiddenc,3,1), Conv(hiddenc,3,3,1)]
        self.model = nn.Sequential(*sequence)
