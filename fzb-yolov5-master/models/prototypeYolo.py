...
class Model(nn.Module):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        self.f1protype = torch.nn.ParameterList([torch.nn.Parameter(data=torch.zeros([7,7],dtype=torch.float), requires_grad=False) for i in self.nc])
        self.f2protype = torch.nn.ParameterList([torch.nn.Parameter(data=torch.zeros([5,5],dtype=torch.float), requires_grad=False) for i in self.nc])
        self.f3protype = torch.nn.ParameterList([torch.nn.Parameter(data=torch.zeros([3,3],dtype=torch.float), requires_grad=False) for i in self.nc])
        self.ROIf1 = RoIAlign(output_size=7, spatial_scale=1., sampling_ratio=-1)
        self.ROIf2 = RoIAlign(output_size=5, spatial_scale=1., sampling_ratio=-1)
        self.ROIf3 = RoIAlign(output_size=3, spatial_scale=1., sampling_ratio=-1)

    def forward(self, x, augment=False, profile=False, visualize=False,callbackDict ={}, domainlabels = 1):
        if augment:
            return self._forward_augment(x)  # augmented inference, None
        return self._forward_once(x, profile, visualize,callbackDict=callbackDict, domainlabels =domainlabels)  # single-scale inference, train
    ...
    def _forward_once(self, x, profile=False, visualize=False, callbackDict ={}, domainlabels = 1):
        y, dt = [], []  # outputs
        dmainThree = []
        for m in self.model:
            ...
            elif (type(m) is DANLayer) or (type(m) is HRDANLayer):
                dmainThree.append(x)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args  考虑添加flag用来输出
        ...
        if m in (Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
            ...
            c2 = ch[f] // args[0] ** 2
        elif m is DANLayer:              # add by fzb
            c1 = ch[f]
            args = [c1,*args]
            out.append(i)
        elif m is IntegDANLayer:              # add by fzb
            args.append([ch[x] for x in f])
