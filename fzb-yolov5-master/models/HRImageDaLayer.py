...
#域自适应网络
class HRDANLayer(nn.Module):

    def __init__(self, inputc,hiddenc = 128):
        super(HRDANLayer, self).__init__()
        self.outfag = True
        sequence = []
        sequence += [GRL(),Conv(inputc,hiddenc), Conv(hiddenc,1),_ImageDA(1)]
        self.model = nn.Sequential(*sequence)
        self.hrLayer = nn.Sequential(*[Conv(inputc,hiddenc), Conv(hiddenc,3), nn.Conv2d(3, 3, kernel_size=1, stride=1,bias=False), nn.Sigmoid()])

    def forward(self, input):
        output = self.model(input)
        hrout = self.hrLayer(input)
        return output, hrout
