...
from utils.hrDecom.torch_utils import initialize_weights, is_parallel, model_info
from utils.hrDecom.yolo_utils import check_anchor_order
...
class Lap_Pyramid_Conv(nn.Module):
    ...
    def pyramid_decom(self, img):
        if img.device != self.kernel.device:
...
class Trans_low(nn.Module):
    def __init__(
        ...
        self.a = nn.Sequential(nn.Conv2d(3,1,3,1,1), nn.ReLU(True))
    ...
    def forward(self, x):
        ...
        # 在这里使用 x + x(1- x)a这是zero-dce的曲线  xe^(ax(1-x))也是可以的 这条曲线

        #  f1,_ = torch.max(x, keep_dim = true)
        #
        #mask = self.trans_guide(torch.cat([x, out], dim=1))

        # ##这里加入自己的代码, 只对### add by fzb begin
        a = self.a(out) + 1
        f1, _ = torch.max(x, dim=1,keepdim=True)
        f2 = f1 + f1*(1 - f1)*a
        f3 = torch.exp(f2) / (torch.exp(f1) + 1e-6)  # 得到倍数
        out = f3 * x
        # saveFakePic(imgs,cat_image, filepath, save_dir, epoch, i)

        saveFakePic(x,out, ['{x}.jpg' for x in range(x.shape[0])],None, 1,1)
        ##########################  add by fzb end
...
class DENet(nn.Module):
    ...
    def forward(self, x):
        ...
        trans_pyrs = []
        trans_pyr, guide = self.trans_low(pyrs[-1])     # 这里是对低通信息增强
