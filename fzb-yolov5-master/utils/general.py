...
def fzbcheck_dataset(data, autodownload=True):
    # Download, check and/or unzip dataset if not found locally
    ...
    if not path.is_absolute():
        path = (ROOT / path).resolve()
    for k in 'strain', 'val', 'test','ttrain':
        if data.get(k):  # prepend path
            data[k] = str(path / data[k]) if isinstance(data[k], str) else [str(path / x) for x in data[k]]

    if data.get('sttrain'):
        data['sttrain'] = str(path / data['sttrain']) if isinstance(data['sttrain'], str) else [str(path / x) for x in data['sttrain']]
    # Parse yaml
    train, val, test, s = (data.get(x) for x in ('train', 'val', 'test', 'download'))
    if val:
        val = [Path(x).resolve() for x in (val if isinstance(val, list) else [val])]  # val path
...
def non_max_suppression(prediction,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        xcIndics = torch.nonzero(xc[xi]).view(-1)
        #fzbx = x[xc[xi]]  # confidence
        x = x[xcIndics]
        ...
        if multi_label:
            ...
            x = torch.cat((box, conf, j.float()), 1)[conf.view(-1) > conf_thres]   # 从前后景的conf置信度 到 类别的置信度
...
def non_max_suppression_tensor(prediction,
                        conf_thres=0.25,
                        iou_thres=0.45,
                        classes=None,
                        agnostic=False,
    ...
    for xi in range(prediction.shape[0]):  # image index, image inference
        ...
        if multi_label:
            ...
            conf, j = x[:, 5:].max(1, keepdim=True)
            x = torch.cat((box, conf, j.float()), 1)   # 从前后景的conf置信度 到 类别的置信度
            #x = torch.cat((box, conf, j.float()), 1)
        # Filter by class
        if classes is not None:
            x = x[(x[:, 5:6] == torch.tensor(classes, device=x.device)).any(1)]
