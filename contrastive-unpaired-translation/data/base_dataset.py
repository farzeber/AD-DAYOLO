    ...
    if os.path.isfile(lb_file):
        ...
        if nl:
            ...
            if len(i) < nl:  # duplicate row check
                ...
                if segments:
                    segments = segments[i]
                msg = f'WARNING: {lb_file}: {nl - len(i)} duplicate labels removed'
        else:
            ne = 1  # label empty
...
def fzb_transform(opt, A_img,A_label_path):

    A_label = get_label(A_label_path)

    nl = len(A_label)  # update after albumentations

    labels_out = torch.zeros((nl, 6))
    if nl:
        labels_out[:, 1:] = torch.from_numpy(A_label)

    if 'resize' in opt.preprocess:
        osize = opt.load_size
        h0, w0 = A_img.size  # orig hw\
        A_img = Image.Image.resize(A_img,(opt.load_size, opt.load_size))
    ...
    if not opt.no_flip:
        if random.random() < 0.5:
            A_img = F.hflip(A_img)
            labels_out[:, 2] = 1 - labels_out[:, 2]
    A_img_tensor = F.to_tensor(A_img)
    A_img_tensor = F.normalize(A_img_tensor, (0.5, 0.5, 0.5), (0.5, 0.5, 0.5))

    return A_img_tensor, labels_out
